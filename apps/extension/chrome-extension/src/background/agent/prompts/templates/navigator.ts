...
- Keep-current organize recovery (once): [{"rewrite_userscript": {"intent": "Repair chatgpt-organize overlay", "script_id": "chatgpt-organize", "source": "<repaired userscript from KEEP_CURRENT_PAYLOAD>"}}, {"run_userscript": {"intent": "Re-run organize with overlay", "script_id": "chatgpt-organize"}}]
...
- Keep-current loop: if a chatgpt-organize run fails with a KEEP_CURRENT Action error (timeout waiting for __nanoChatGptOrganize.done, or contract/action drift from seed), the accompanying Action result KEEP_CURRENT_PAYLOAD is the same overlay or seed bytes Chrome just ran — not a successful organize. Repair those bytes, keep the same contract tokens and same-origin fetch title PATCH semantics, call rewrite_userscript with the repaired overlay, then run_userscript once. Do not rewrite for signed-out, 401, origin lock, or overlapping organize. If KEEP_CURRENT already_retried, stop; do not loop.
