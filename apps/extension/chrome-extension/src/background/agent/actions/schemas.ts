...
export const runUserscriptActionSchema: ActionSchema = {
  ...
  description:
    'Register and run a reviewed userscript payload in the page MAIN world, scoped to the current tab origin. script_id is a reviewed-id enum: "fixture" (banner/counter inject proof) or "chatgpt-organize" (same-origin fetch on chatgpt.com / chat.openai.com: list scrap chats and PATCH titles; signed-out or 401 is an error). Do not use click_element to organize ChatGPT chats.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    script_id: z
