...
export const runUserscriptActionSchema: ActionSchema = {
  ...
  description:
    'Run a reviewed userscript payload in the page MAIN world, scoped to the current tab origin. Packaged public/userscripts/*.user.js files are the reviewed seed. If a rewrite_userscript overlay exists for script_id, inject that stored source via executeScript func/args (MAIN world); otherwise inject the packaged seed. This is not registerContentScripts for rewrite — overlays never write packaged files. script_id is a reviewed-id enum: "fixture" (banner/counter inject proof) or "chatgpt-organize" (one-shot executeScript on chatgpt.com: list scrap chats and PATCH titles; signed-out or 401 is an error). Do not use click_element to organize ChatGPT chats.',
  schema: z.object({
    intent: z.string().default('').describe('purpose of this action'),
    script_id: z
